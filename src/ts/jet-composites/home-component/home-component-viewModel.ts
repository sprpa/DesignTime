  ...
  implements Composite.ViewModel<Composite.PropertiesType>
{
  ...
  multiSelectDataProvider: ArrayDataProvider<
    string,
  ...
  constructor(context: Composite.ViewModelContext<Composite.PropertiesType>) {
    ...
    const rolesList = [{ id: "1", value: "creator", name: "Creator" }];

    this.roledata = new ArrayDataProvider(rolesList, {
      keyAttributes: "value",
    });
    ...
    this.multiSelectDataProvider = new ArrayDataProvider(this.optionsList, {
      keyAttributes: "value",
    });
  ...
  onStartClick = async (): Promise<void> => {
    try {
      const response = await fetch(
        "http://10.26.1.52:5010/generate-login-url",
        {
          method: "GET",
          headers: {
  ...
  selectProfileFiles(event: ojButton.ojAction): void {
    pickFiles(this.selectProfileListener, {
      //  Use `pickFiles` instead of `FilePickerUtils.pickFiles`
